...
function 二级(cmstype){
    ...
    let limit_text = info.substring(0,parseInt(lsg.getItem('简介字数','900')))+'';
    let rich_html = ("剧情简介：\t\t\t\t\t"+"<br> ᅟ      ‌‍‎‏ ").fontcolor("#098AC1")+limit_text.fontcolor("grey").small();
    rich_html = rich_html+"查看".fontcolor("#098AC1").small().link($().b64("'").rule(d => setResult(d), [{
        title:info,
        col_type:"long_text"
    }]));
    d.push({
    ...
    if(v1||app||iptv){//v1或者app,iptv都有魔断开关
        d.push({
            title:small(color('若无法正常播放可尝试切换此选项：','#098AC1'))+color(lsg.getItem('dpswitch','否'),'#d96715'),
    ...
    let showOrder = getMyVar('顺序','正序')==='逆序'?color('','#228be6'):color('','#d0aa344');
    ...
    d.push({
        title:small(color('💐选集：共','#098AC1')+color(tabs.length,'#d96715')+color('处源','#098AC1')+color('〔正版之源方有郸幕〕','#FA2800'))+blank()+right(small(showOrder)),
        col_type:'text_1',
        url:$('#noLoading#').lazyRule(()=>{
        ...
        d.push({
            ...
            url:$('#noLoading#').lazyRule((i,tab_name,nowTab)=>{
                if(nowTab===parseInt(i)){
                    return 'toast://Zyou牢骚:吃饱了没事儿干?点什么点!'
                }
                putMyVar('nowTab',''+i);
                refreshPage(false);
    ...
    if(list.length>Number(lsg.getItem('每页数量',100))&&canPage) {
        var showCol = 'avatar';
        var showPic = '';
        var pageTitle = '';
        function setTitle(){
            if(showCol==='avatar'){
                pageTitle = pageTitle.replace(/““””/g,'');
                showPic = 'https://jihulab.com/czy2/czy01/-/raw/main/Czyou/sK.png';
            }
        }
        if (getMyVar('选集显示', '分页') === '分页') {
            pageTitleInfo = color('页显:','#585858')+color('$cnt', '#d96715')+color('集，共:','#585858')+color(list.length, '#d96715')+color('集  第:','#585858') + color('$page'+'/'+最大页数, '#d96715')+color('页','#585858');
