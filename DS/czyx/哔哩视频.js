var rule = {
	...
	lazy: $js.toString(() => {
		...
		let response = JSON.parse(request("http://niiuma.qi-simple.top/niuma.php?url=" + input));
	...
	lazy: `js:
        if (/^http/.test(input)) {
            let dan = 'http://222.186.57.24:7600/dm/dm.php?url=' + input;
            input = {
                jx: 1,
                danmaku: dan,
                url: input,
